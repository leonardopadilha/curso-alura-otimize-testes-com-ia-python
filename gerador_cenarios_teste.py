...
def gerar_cenario_teste(caso_uso, documento, dicionario_arquivos, assistente, thread, modelo=MODELO_GPT_4):
    pergunta = f"""
        Você é um especialista em desenvolver cenários de teste para validar uma aplicação web, quanto 
        sua navegação. Para isso, considere o caso de uso destacado em: {caso_uso}.

        Além disso, você também deve utilizar os documentos enviados pelo usuário para elaboração do teste.
        Consulte os documentos internos buscando: {dicionario_arquivos[documento+".html"]}, {dicionario_arquivos[documento+".css"]}  
        e {dicionario_arquivos[documento+".js"]} para garantir o uso adequado dos componentes que irão compor o teste.
    ...
    while ran.status != STATUS_COMPLETED:
        print("Gerando Cenário: ", ran.status)
        ran = cliente.beta.threads.runs.retrieve(
            thread_id = thread.id,
            run_id = run.id
        )
